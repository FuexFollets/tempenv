...
namespace tempenv {
    configuration_file::configuration_file(const nlohmann::json& configuration_json_file,
            const std::vector<std::string>& chosen_preset_names) :
        ...
        // Configuration file does not contain presets section
        if (!configuration_json_file.contains("presets")) { return; }
        ...
        // Separate valid and invalid chosen presets
        for (const std::string& chosen_preset: chosen_preset_names) { 
        ...
        for (const std::string& valid_preset: valid_presets) {
            ...
            const auto paths_to_copy_with {configured_preset.at("copy-with")
                .get<std::vector<std::filesystem::path>>()};

            const auto executed {configured_preset.at("execute-in-test-directory")
                .get<std::vector<std::vector<std::string>>>()};

            _copy_with.insert(_copy_with.end(), paths_to_copy_with.begin(),
                    paths_to_copy_with.end());

            _execute_in_test_directory.insert(_execute_in_test_directory.begin(), executed.begin(),
                    executed.end());
        }
    }
